...
const Appointment = () => {
  ...
  const handleAppointmentStatus = (e, statusId) => {
    ...
    axios
      .patch(
        `https://mysterious-bastion-18790.herokuapp.com/update-status/${statusId}`,
        {
  ...
  const handleCancelAppointment = (id) => {
    ...
    if (proceed) {
      const url = `https://mysterious-bastion-18790.herokuapp.com/cancelAppointments/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
          ...
          if (data.deletedCount > 0) {
            ...
            const remainingOrder = cancelAppointments.filter(
              (order) => order._id !== id
            );
                          ...
                          <Box sx={{ minWidth: 120 }}>
                            <FormControl sx={{ m: 1, minWidth: 120 }}>
                              ...
                              >
                                ...
                                <MenuItem
                                  onClick={(e) =>
                                    handleAppointmentStatus(e, row._id)
                                  }
                                  value={20}
                                >
                                  Pending
