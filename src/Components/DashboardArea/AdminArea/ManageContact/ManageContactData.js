...
const ManageContactData = ({ contact }) => {
  ...
  const handleDeleteContact = (id) => {
    ...
    if (proceed) {
      const url = `https://mysterious-bastion-18790.herokuapp.com/deleteContacts/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
          ...
          if (data.deletedCount > 0) {
            ...
            const remainingContact = deleteContact.filter(
  ...
  return (
    <TableRow hover role="checkbox" tabIndex={-1}>
      ...
      <TableCell className="user-name">{userName}</TableCell>
