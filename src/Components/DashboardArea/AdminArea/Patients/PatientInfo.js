...
const PatientInfo = ({ patient }) => {
  ...
  const handleDeleteUser = (id) => {
    ...
    if (proceed) {
      const url = `https://mysterious-bastion-18790.herokuapp.com/appointments/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
          ...
          if (data.deletedCount > 0) {
            ...
            const remainingOrder = deleteUser.filter(
              (order) => order._id !== id
            );
  ...
  return (
    <>
      <TableRow hover role="checkbox" tabIndex={-1}>
        <TableCell className="user-name">{patient._id.slice(2, 8)}</TableCell>
        ...
        <TableCell>{patient.patient_weight} KG</TableCell>
        ...
        <TableCell>
          {" "}
          {patient.payment ? (
            <div className="success-text">Paid</div>
          ) : (
            <div className="text-alert">Not Paid</div>
          )}{" "}
        </TableCell>
        ...
        <TableCell>
          <Link
            to={`/dashboard/meetlink/${patient._id}`}
        ...
        <TableCell>
          <Link
            to={`/dashboard/prescription/${patient._id}`}
            className="tbl-btn-primary"
          >
            Send Rx
          </Link>
