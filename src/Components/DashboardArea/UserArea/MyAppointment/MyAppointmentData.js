      ...
      fetch(url, {
        ...
        .then((data) => {
          ...
          if (data.deletedCount > 0) {
            ...
            const remainingAppointment = deleteAppointment.filter(
      ...
      <TableRow hover role="checkbox" tabIndex={-1}>
        ...
        <TableCell>
          ...
          {appointment.payment ? (
            ...
            >
              {" "}
              <span className="tbl-btn-alert">Pay Fast</span>{" "}
            </Link>
          )}{" "}
        </TableCell>
        {/* meetlink */}
        <TableCell>
          {appointment.meetingLink === "Please Wait For Link" ? (
            <div className="text-warning">Please Wait</div>
          ) : (
            <input
              onClick={handleOpen}
              class="btn tbl-btn-primary my-2"
              type="submit"
              value="Get Link"
            />
          )}
        ...
        {/* prescription */}
        <TableCell>
          {appointment.prescriptions === "Writing " ? (
            <div className="text-warning">Please Wait</div>
          ) : (
            <input
              onClick={handlePresOpen}
              class="btn tbl-btn-primary my-2"
              type="submit"
              value="View"
            />
